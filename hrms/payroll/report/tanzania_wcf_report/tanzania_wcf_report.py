...
from hrms.payroll.report.tanzania_statutory_utils import get_salary_slips
...
def validate_filters(filters):
	for f in ("payroll_month", "company"):
...
def get_columns():
	return [
		...
		{"label": _("WCF (0.5%)"), "fieldname": "wcf", "fieldtype": "Currency", "width": 130},
...
def get_data(filters):
	...
	wcf_rate = flt(filters.get("wcf_rate") or 0.5) / 100.0

	data = []
	for slip in slips:
		wcf = flt(slip.gross_pay) * wcf_rate
		data.append({
...
def get_summary(data):
	...
	return [
		{"label": _("Total Gross Pay"), "value": sum(r["gross_pay"] for r in rows), "datatype": "Currency", "indicator": "blue"},
		{"label": _("Total WCF Payable"), "value": sum(r["wcf"] for r in rows), "datatype": "Currency", "indicator": "orange"},
